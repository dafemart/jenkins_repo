var currentRow = 1;
var currentRequestToRemoveID = "0";
var btn;
...
function displayRequestPortal()
{
     location.href = "http://localhost:8095/TRMS_WORKING/requestForm.html";
} 
...
function deleteRequest(event){
	...
	if ( target.nodeName != 'TD' )
	 ...
	 btn = target;
	 currentRequestToRemoveID = columns[0].innerHTML;
}

function sendDeletionRequest(){
	if(currentRequestToRemoveID == "0"){
	...
	xhr.open("DELETE", "EmployeeServlet?requestid=" + currentRequestToRemoveID, true);
	xhr.send();
}
...
window.onload = function() {
	document.getElementById("submit").addEventListener("click", displayRequestPortal, false);
	document.getElementById("delete").addEventListener("click", sendDeletionRequest, false);
